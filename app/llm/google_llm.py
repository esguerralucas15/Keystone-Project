...
try:
    ...
    # escribir el traceback a un archivo para diagnóstico
    try:
        with open('llm_import_error.log', 'w', encoding='utf-8') as fh:
            fh.write(_import_error)
    except Exception:
        pass
...
def get_llm():
    ...
    if ChatGoogleGenerativeAI is None:
        raise RuntimeError("Dependencia 'langchain_google_genai' no encontrada. Instala las dependencias.")
    ...
    _llm = ChatGoogleGenerativeAI(
        ...
        def _ensure_imports():
            ...
            try:
                ...
                from langchain_core.messages import HumanMessage as _HumanMessage, SystemMessage as _SystemMessage, AIMessage as _AIMessage
        ...
        def get_llm():
            ...
            err = _ensure_imports()
            if err:
                raise RuntimeError("Paquetes LLM no disponibles. Instala 'langchain_google_genai' y 'langchain-core'.\n" + err)
        ...
        def _fallback_reply(user: Dict[str, Any], profile: Dict[str, Any], message: str) -> str:
            ...
            if profile:
                ...
                parts.append("1) Reduce gastos variables; 2) Prioriza pago de deudas con mayor interés; 3) Intenta ahorrar al menos 5-10% de ingresos.")
            else:
                parts.append("Provee tu información financiera (ingresos, gastos, deudas) para recomendaciones personalizadas.")
            return ' '.join(parts)
